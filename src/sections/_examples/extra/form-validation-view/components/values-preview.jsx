...
export function ValuesPreview({ sx, onCloseDebug }) {
        ...
        <Group label="State" sx={{ color: 'success.light' }}>
          {[
            ...
            <div key={item} className={classes.item}>
              ...
              <span className={classes.value}>{JSON.stringify(formState[item], null, 2)}</span>
            </div>
          ))}
        </Group>
...
function parseValue(values, key) {
  ...
  if (key === 'multiUpload') {
    return JSON.stringify(
      values.multiUpload.map((file) => fileData(file)),
      null,
      2
    );
  }
