...
export function ProductFiltersDrawer({ open, onOpen, onClose, canReset, filters, options }) {
  ...
  const renderColor = () => (
    <Box sx={{ display: 'flex', flexDirection: 'column' }}>
      ...
      <ColorPicker
        ...
        onChange={(colors) => handleFilterColors(colors)}
        limit={6}
      />
    </Box>
  );
...
function InputRange({ type, value, onChange: onFilters }) {
